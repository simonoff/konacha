  ...
  describe 'suite/test links' do
    ...
    describe 'suite link' do
      ...
      it 'should contain spec path and grep string' do
        expect(subject['href']).to eq("#{page.current_url}#{spec_file}?grep=#{URI.encode(suite_name)}")
      end
    end

    describe 'replay link (arrow icon)' do
      subject { page.find(%(      a[href="#{page.current_url}#{spec_file}?grep=#{URI.encode([suite_name, test_name].join(' '))}"]      )) }
